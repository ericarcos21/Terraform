...
resource "azurerm_resource_group" "test_cloudazure" {
  name     = "Infraestructura-Cloud-Arcos"
  location = "eastus"
}
...
resource "azurerm_private_dns_zone" "dns_zone" {
  ...
  name                = "bdd.arcos.mysql.database.azure.com"
  resource_group_name = azurerm_resource_group.test_cloudazure.name
}
...
resource "azurerm_mysql_flexible_server" "server_mysql" {
  ...
  name                         = "mysqlarcos"
  resource_group_name          = azurerm_resource_group.test_cloudazure.name
  administrator_login          = "arcos"
  administrator_password       = "Arcos2000."
...
resource "azurerm_public_ip" "test_cloudazure" {
  ...
  sku                 = "Standard"
  domain_name_label   = "machineae${count.index}"
}
...
resource "azurerm_virtual_machine" "test_cloudazure" {
  ...
  os_profile {
    computer_name  = "Arcos"
    admin_username = "Arcos"
    admin_password = "Arcos2000."
  }
